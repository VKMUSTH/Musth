	...
	<table border=0 cellpadding=0 cellspacing=0 >
		<TR class="niv1">
			<TD colspan=12 ><B>CLIENTS DU MOIS EN COURS</B></TD>
		</tr>
		<TR class="niv2">
	...
	<?php
	// Déclaration des variables de tot
	//$fMargeTotGeneral = 0;
	//$fPdvTotGeneral = 0;
	?>
	
	<?php// Récupération des clients
		$get_client = $bdd->prepare('SELECT
			numclient,
			numcontact,
	...
	<?php	// Récupération des données de lj	
		$get_cotation = $bdd->prepare('SELECT 
			numclient,
			numitem,
			position,
			designation,
			...
			');	
		$get_cotation->execute();
		$cotation = $get_cotation -> fetchAll();
		$get_cotation->closeCursor(); 
	...
	?>
		...
		<tr class="niv3">
			...
			<?$get_contact = $bdd->prepare('SELECT * FROM contacts WHERE numcontact = '.$cli['numcontact'].'');
			$get_contact->execute(array($cli['numcontact']));
