	...
	<table 		name="calendrier"			border=0 cellpadding=0 cellspacing=0 >
		<tr 	name="titre_calendrier" 	class="niv1">
			<TD COLSPAN=12 ALIGN=CENTER VALIGN=MIDDLE>	<B>BITCOINWING</B></TD>
		</tr>
		<tr 	name="mois_calendrier" 		class="niv2">
			...
			<?php
			$sql = "SELECT	DAY(date) AS jour, MONTH(date) AS mois, YEAR(date) AS annee, DATE(date) AS date_jui, designation, prix,quantite,ca,gain, revenu_journalier,reste FROM falcoon WHERE MONTH(date) = \"01\" AND YEAR(date) = '".$admin['annee_en_cours']."' AND designation= \"BITCOINWING\"
 ORDER BY date ASC ";
