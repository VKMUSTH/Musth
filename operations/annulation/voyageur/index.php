	...
	while($titre = mysql_fetch_assoc($req1)) { ?>
			<td> 	<h1>Annulation de Voyageurs:<?php echo $titre['titre']; ?> </h1>
			<?php } //$get_titre ?>
	...
	<table border=0 cellpadding=0 cellspacing=0 >
		<TR class="niv1">
			<TD  COLSPAN=8 ALIGN=CENTER VALIGN=MIDDLE >				<B>LISTE DES VOYAGEURS</B>	</TD>
		</TR>
	...
	<?php
	$sql2 = 'SELECT * FROM voyageurs WHERE numclient = '.$admin['numclient'].' AND attribut =\'voyageur\' ';
	...
	while($contacts = mysql_fetch_assoc($req3)) { ?>
				<form name="setnumvoyageur" action="goto_contact.php" method="post" >
